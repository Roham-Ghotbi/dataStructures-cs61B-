...
public class ArrayHeap<T> implements ExtrinsicPQ<T> {
    ...
    private void swim(int index) {
        ...
        if (getNode(parentIndex(index)) == null || node == null) { //if root
            return;
        }
        if (getNode(parentIndex(index)).priority() > node.priority()) {
    ...
    private void sink(int index) {
        ...
        int minSides = min(left, right);
        int newLocation = min(minSides, index);

        if (newLocation != index) {
            swap(newLocation, index);
            sink(newLocation);
        }
